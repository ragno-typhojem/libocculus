...
const LoginScreen = () => {
  ...
  useEffect(() => {
    const checkAdBlock = async () => {
      try {
        await fetch('https://firestore.googleapis.com/favicon.ico', {
          method: 'HEAD',
          mode: 'no-cors'
        });
      } catch (err) {
          ...
          <div className="p-8 pt-0 space-y-4">
            ...
            {showAdBlockWarning && (
              <motion.div
                ...
                className="bg-orange-50 border border-orange-500 rounded-lg p-4"
              >
                <div className="flex items-start gap-2">
                  <ShieldAlert className="h-5 w-5 text-orange-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-orange-800 text-sm font-semibold">AdBlock Algılandı</p>
                    <p className="text-orange-700 text-xs mt-1">
                      Lütfen AdBlock veya benzeri uzantıları bu site için devre dışı bırakın.
                    </p>
                  </div>
                </div>
              </motion.div>
